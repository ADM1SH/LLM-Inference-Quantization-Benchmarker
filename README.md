...
[![Ollama Support](https://img.shields.io/badge/Ollama-Active-orange.svg)](https://ollama.com/)
[![macOS Apple Silicon](https://img.shields.io/badge/Platform-macOS%20Apple%20Silicon-lightgrey.svg)]()
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](https://opensource.org/licenses/MIT)

An automated, local framework for systematically profiling the performance, memory utilization, and response quality of quantized Large Language Models (LLMs) on edge devices.
...
# 2. Make the orchestrator executable and launch it
chmod +x run.sh
./run.sh
```

The script will setup the virtual environment, pull the judge model (`llama3.2:3b`), compile GGUF precision variants for Qwen2.5-0.5B-Instruct, run the profiling suite, and spin up an interactive dashboard at: **[http://localhost:8000](http://localhost:8000)**.
...
.
├── .gitignore
├── README.md               # You are here (Repository & Paper Documentation)
├── requirements.txt        # Python libraries
├── run.sh                  # Virtual environment setup & web server runner
├── benchmark.py            # Primary Python benchmarking engine
├── data/                   # Directory containing downloaded GGUF files (ignored)
├── venv/                   # Python Virtual Environment directory (ignored)
└── web/                    # Dashboard UI Web Application
    ├── index.html          # Core dashboard HTML structural file
    ├── style.css           # Glassmorphic custom styling sheet
    ├── app.js              # Fetching and rendering Chart.js visualizations
    └── results/            # Compilation directory for outputs
        ├── benchmark_results.json # Raw benchmark stats
        ├── benchmark_results.csv  # Flattened metric format
        └── benchmark_summary.png  # Generated static visualization charts
```

### Core Architecture Workflow
```mermaid
graph TD
    A[Start: run.sh] --> B[Set up Python venv & Install dependencies]
    B --> C[Launch Ollama Service API]
    C --> D[Run benchmark.py]
    D --> E[Check GGUF models in data/]
    E -- Missing GGUF --> F[Download from HuggingFace Hub]
    E -- Present GGUF --> G[Register model in Ollama CLI]
    F --> G
    G --> H[Run Pre-warm Inference Call]
    H --> I[Execute Prompt Suite across Q4, Q8, FP16]
    I --> J[Launch SystemResourceMonitor thread]
    J --> K[Query /api/generate for tokens and speed]
    J --> L[Query /api/ps for peak VRAM/RAM allocation]
    K --> M[Invoke Judge LLM: llama3.2:3b]
    L --> M
    M --> N[Export JSON & CSV to web/results/]
    N --> O[Generate Matplotlib & Seaborn static charts]
    O --> P[Start local dashboard web server on Port 8000]
```

---

## 3. Detailed Setup & Installation Guide

### Prerequisites
*   **macOS** (Optimized for Apple Silicon Unified Memory).
*   **Python 3.10+** (Python 3.14 recommended).
*   **Ollama** installed on your system. Verify that it is available on your shell PATH by running `which ollama`.

### Installation Steps

1. **Clone the Repository:**
   ```bash
   git clone https://github.com/your-username/LLM-Inference-Quantization-Benchmarker.git
   cd LLM-Inference-Quantization-Benchmarker
   ```
...
2. **Verify/Setup Ollama Service:**
   ...
   ollama serve > /tmp/ollama.log 2>&1 &
   ```
   Verify Ollama is active and responsive:
   ```bash
   curl http://localhost:11434/api/tags
   ```

3. **Install the Judge Model:**
   The quality metric evaluates responses using a local LLM-as-a-Judge. Pull the default judge model:
   ```bash
   ollama pull llama3.2:3b
   ```

4. **Initialize Python Virtual Environment and Install Dependencies:**
...
5. **Run the Profiling Suite:**
   Execute the core benchmarker:
   ```bash
   python benchmark.py
   ```

6. **Serve the Interactive Dashboard:**
   Run the static web server to view interactive graphical results:
   ```bash
...
### INTRODUCTION
Modern LLM inference dynamics are governed by two distinct computational phases: the compute-bound prefill phase, where attention mechanisms dominate FLOPs, and the memory-bandwidth-bound autoregressive decoding phase, where token generation is constrained by VRAM bandwidth and cache coherence. The latter phase, critical for real-time applications, is particularly sensitive to quantization-induced precision loss. The quantization frontier has evolved from naïve FP16/BF16 execution to sophisticated Post-Training Quantization (PTQ) techniques, including symmetric/asymmetric INT8, 4-bit GPTQ/AWQ, and sub-byte GGUF configurations. These methods reduce memory footprints by 4–8× while introducing non-linear trade-offs in throughput, numerical stability, and model quality.

#### The Problem
Developers face a systemic challenge in systematically mapping these trade-offs:
...
*   **vLLM:** Optimizes throughput via PagedAttention but lacks granular quantization profiling and quality metrics.
...
### MATHEMATICAL FORMULATION & PROFILING METRICS
...
### EXPERIMENTAL WORKFLOW & SIMULATED EXECUTION MATRIX
...
### SYSTEM DYNAMICS & DEEP TRADE-OFF ANALYSIS
...
#### Test Case Case Study: The Bat & Ball Riddle
...
## 5. References & Links
*   [benchmark.py Source Code](file:///Users/adamanwar/Desktop/Coding%20Projects/LLM-Inference-Quantization-Benchmarker/benchmark.py)
*   [Dashboard Application Core js](file:///Users/adamanwar/Desktop/Coding%20Projects/LLM-Inference-Quantization-Benchmarker/web/app.js)
*   [Raw JSON Dataset](file:///Users/adamanwar/Desktop/Coding%20Projects/LLM-Inference-Quantization-Benchmarker/web/results/benchmark_results.json)
*   [Matplotlib Static Output Chart](file:///Users/adamanwar/Desktop/Coding%20Projects/LLM-Inference-Quantization-Benchmarker/web/results/benchmark_summary.png)
